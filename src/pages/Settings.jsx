      ...
      <div className="card">
        ...
        {activeTab === 'title' && (
          <div>
            {user && user.role === 'admin' && (
              <>
                <div style={{ marginBottom: '1rem', padding: '1rem', background: '#e3f2fd', borderRadius: '8px' }}>
                  <strong>ℹ️ Admin Mode:</strong> These are global default title settings. Users can override these with their own preferences.
                </div>
                {message && (
                  <div className={`alert ${message.includes('success') ? 'alert-success' : 'alert-error'}`}>
                    {message}
            ...
            <form onSubmit={user && user.role === 'admin' ? handleSubmit : handleUserSettingsSubmit}>
              {(() => {
                const isAdmin = user && user.role === 'admin';
                const currentSettings = isAdmin ? settings : userSettings;
                const handleSettingsChange = isAdmin ? handleChange : handleUserSettingsChange;
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                  <input
                    ...
                    id="title_bg_color"
                    className="form-control"
                    value={currentSettings.title_bg_color || '#000000'}
                    onChange={(e) => handleSettingsChange('title_bg_color', e.target.value)}
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                  <input
                    ...
                    min="0"
                    max="100"
                    value={currentSettings.title_opacity || '80'}
                    onChange={(e) => handleSettingsChange('title_opacity', e.target.value)}
                    style={{ flex: 1 }}
                  />
                  <input
                    type="number"
                ...
                <small style={{ color: '#7f8c8d', fontSize: '0.85rem' }}>
                  0 = Transparent, 100 = Opaque. Current: {currentSettings.title_opacity || '80'}%
                </small>
              </div>
              ...
              <div className="form-group">
                ...
                <select
                  id="title_position"
                  className="form-control"
                  value={currentSettings.title_position || 'bottom-left'}
                  onChange={(e) => handleSettingsChange('title_position', e.target.value)}
                >
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                  <input
                    ...
                    id="title_text_color"
                    className="form-control"
                    value={currentSettings.title_text_color || '#FFFFFF'}
                    onChange={(e) => handleSettingsChange('title_text_color', e.target.value)}
                    style={{ width: '80px', height: '40px', cursor: 'pointer' }}
                  />
                  <input
                    type="text"
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                  <input
                    ...
                    min="0"
                    max="20"
                    value={currentSettings.title_box_padding || '5'}
                    onChange={(e) => handleSettingsChange('title_box_padding', e.target.value)}
                    style={{ flex: 1 }}
                  />
                  <input
                    type="number"
                ...
                <small style={{ color: '#7f8c8d', fontSize: '0.85rem' }}>
                  Padding around text inside the box. Lower = less CPU usage. Current: {currentSettings.title_box_padding || '5'}px
                </small>
              </div>

              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={isAdmin ? saving : savingUserSettings}>
                  {isAdmin ? (saving ? 'Saving...' : 'Save Settings') : (savingUserSettings ? 'Saving...' : 'Save My Settings')}
                </button>
              </div>
              </>
                );
            ...
            <div style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '1px solid #e1e8ed' }}>
              ...
              <div style={{
                ...
                minHeight: '200px',
                display: 'flex',
                alignItems: (user && user.role === 'admin' ? settings : userSettings).title_position?.startsWith('top') ? 'flex-start' : 'flex-end',
                justifyContent: (user && user.role === 'admin' ? settings : userSettings).title_position?.includes('center') ? 'center' : (user && user.role === 'admin' ? settings : userSettings).title_position?.includes('right') ? 'flex-end' : 'flex-start'
              }}>
                <div style={{
                  backgroundColor: (user && user.role === 'admin' ? settings : userSettings).title_bg_color || '#000000',
                  opacity: ((user && user.role === 'admin' ? settings : userSettings).title_opacity || 80) / 100,
                  padding: '1rem 1.5rem',
                  borderRadius: '4px',
                  color: (user && user.role === 'admin' ? settings : userSettings).title_text_color || '#FFFFFF',
                  fontSize: `${(user && user.role === 'admin' ? settings : userSettings).title_font_size || 32}px`,
                  fontWeight: 'bold',
                  maxWidth: '80%',
                  wordWrap: 'break-word'
                }}>
